...
// PICK YOUR BOARD VERSION HERE
//#define CONFIG_CAMERA_PAD_ESP32_S2_KALUGA_V1_1
//#define CONFIG_LCD_PAD_ESP32_S2_KALUGA_V1_2
#define CONFIG_LCD_PAD_ESP32_S2_KALUGA_V1_3

// PICK YOUR LDC MODEL HERE
...
// IDF Components were made part of this sketch 
#include "lcd.h"
#include "board.h"
...
void esp_photo_display(void)
{
    ...
    uint8_t *rgb565 = (uint8_t *)malloc(IMAGE_WIDTH * IMAGE_HIGHT * 2);
    if (NULL == rgb565) {
        ESP_LOGE(TAG, "can't alloc memory for rgb565 buffer");
        return;
    }
    uint8_t *buf = (uint8_t *)malloc(IMAGE_MAX_SIZE);
    if (NULL == buf) {
        free(rgb565);
...
void loop() {
  ...
  // NONE -- everything is done in app_main_arduino()
}
